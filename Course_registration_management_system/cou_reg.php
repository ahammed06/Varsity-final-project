...
if(isset($_REQUEST["submit"]))
{
	...
	$sql8= "select stu_offer.course_code, stu_offer.day1, stu_offer.day2, stu_offer.day3, stu_offer.time1, stu_offer.time2, stu_offer.time3, course.credit from stu_offer,course
	   where stu_offer.course_code=course.course_code AND stu_offer.student_id='$id' AND stu_offer.s_name='$s_name' AND stu_offer.year='$year' AND (status='accepted' OR status='pending')";
...
$credit_hour1+=$hour1;
	...
	foreach($_REQUEST["add"] as $values){
	   ...
	   where student_id='$id' AND s_name='$s_name' AND year='$year'";
...
$credit_hour+=$hour;
	...
	$sql6= "select course_code from stu_offer
	   where course_code='$course_code' AND intake='$intake' AND section='$section' AND s_name='$s_name' AND year='$year' AND department='$department' AND status='accepted'";
$result6 =$conn-> query($sql6);
$count=mysqli_num_rows($result6);

$sql7= "select student_id,course_code from stu_offer
	   where student_id='$id' AND course_code='$course_code' AND s_name='$s_name' AND year='$year' AND (status='accepted' OR status='pending')";
...
<?php
	foreach($_POST["add"] as $values){
		...
		?>
<td><input type="checkbox" name="add[]" value="<?php echo $course_code; ?> <?php echo $day1; ?> <?php echo $day2; ?> <?php echo $day3; ?> <?php echo $time1; ?> <?php echo $time2; ?> <?php echo $time3; ?>" checked></td>
<?php
echo "</tr>";
...
if($a==1){
	...
	$sql1="INSERT INTO stu_offer(student_id,s_name,year,intake,section,department,course_code,day1,day2,day3,time1,time2,time3)
	       VALUES ('$id','$s_name','$year','$intake','$section','$department','$course_code','$day1','$day2','$day3','$time1','$time2','$time3')";
