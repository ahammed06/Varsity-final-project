...
$year=$row1["year"];
	...
	$sql="UPDATE stu_offer SET status='dropped' WHERE id = '$acc_id' AND status='accepted' AND s_name='$s_name' AND year='$year'";

	if ($conn->query($sql) === TRUE) {
		
		$m="Record Dropped!!!";
	   echo "<script type='text/javascript'>alert('$m');</script>";
	   header("refresh:1; url=accepted_course_detail.php?accept=$stu_id&receipt=$receipt_id");
	} else {
