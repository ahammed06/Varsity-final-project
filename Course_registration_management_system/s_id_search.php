...
$year=$row8["year"];
							...
							$sql = "select student_id,name,gender,email,intake,section,mobile,department,image from student where student_id='$student_id' ORDER BY department ASC, intake DESC, section ASC, student_id ASC";
...
$sql9="SELECT DISTINCT student_id FROM stu_offer WHERE student_id='$student_id' AND s_name='$s_name' AND year='$year' AND status='accepted'";
	$result9 =$conn-> query($sql9);
if($result9-> num_rows > 0){
	$sql5= "select DISTINCT stu_offer.student_id, stu_offer.status,
	   receipt.receipt_no from receipt,stu_offer 
	   where stu_offer.student_id='$student_id' AND stu_offer.student_id=receipt.student_id AND stu_offer.s_name='$s_name'
	   AND stu_offer.year='$year' AND stu_offer.status='accepted' AND stu_offer.s_name=receipt.semester AND stu_offer.year=receipt.year";
