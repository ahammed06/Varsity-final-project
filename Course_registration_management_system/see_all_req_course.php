...
<?php
$sql2= "select DISTINCT department from stu_offer WHERE s_name='$semester' AND s_name='$sem' AND year='$year' AND year='$yea' AND department='$dep' AND student_id='$stu_id' AND status='accepted' ORDER BY department ASC,year DESC,s_name DESC,intake DESC,section ASC,student_id ASC";
...
<p align="center"><u><B><a href="see_teach_routine.php?id=<?php echo $stu_id; ?>&semester=<?php echo $semester; ?>&year=<?php echo $year; ?>" class="btn btn-danger">See Routine</a></b></u></p>

<?php
$sql8= "select DISTINCT student_id from stu_offer WHERE s_name='$semester' AND s_name='$sem' AND year='$year' AND year='$yea' AND department='$department' AND department='$dep' AND student_id='$stu_id' AND status='accepted' ORDER BY department ASC,year DESC,s_name DESC,intake DESC,section ASC,student_id ASC";
...
$sql= "select stu_offer.id, stu_offer.s_name, stu_offer.year,stu_offer.intake,stu_offer.section,stu_offer.department,stu_offer.course_code,stu_offer.student_id, stu_offer.status,
	   ...
	   AND stu_offer.student_id='$stu_id' AND receipt.year=stu_offer.year AND receipt.semester=stu_offer.s_name
	   ORDER BY course_code ASC,department ASC,year DESC,s_name DESC,intake DESC,section ASC,student_id ASC";
...
<?php 
	while($row=$result->fetch_assoc()){
		...
		?>
		<td><a href="drop_course_acc.php?delete=<?php echo $row["id"]; ?>&see=<?php echo $stu_id; ?>&receipt=<?php echo $receipt_id; ?>" class="btn btn-danger">Drop</a></td>
		<?php
echo "</tr>";
	}
	echo "</table>";
...
<a href="drop_all_course.php?delete=<?php echo $stu_id; ?>&receipt=<?php echo $receipt_id; ?>" class="btn1 btn-danger">Drop All</a>
</div>
